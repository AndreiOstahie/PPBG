...
void Lab9::Init()
{
    ...
    // Create a shader program for surface generation
    {
        ...
        shader->AddShader(PATH_JOIN(window->props.selfDir, "src/lab", "lab9", "shaders", "VertexShader.glsl"), GL_VERTEX_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, "src/lab", "lab9", "shaders", "GeometryShader.glsl"), GL_GEOMETRY_SHADER);
        shader->AddShader(PATH_JOIN(window->props.selfDir, "src/lab", "lab9", "shaders", "FragmentShader.glsl"), GL_FRAGMENT_SHADER);
...
void Lab9::RenderMeshInstanced(Mesh* mesh, Shader* shader, const glm::mat4& modelMatrix, int instances, const glm::vec3& color)
{
    ...
    int location = glGetUniformLocation(shader->program, "no_of_generated_points");
    glUniform1i(location, no_of_generated_points);

    // Surface type
    location = glGetUniformLocation(shader->program, "surface_type");
    glUniform1i(location, surface_type);

    // Max translation and rotation
    location = glGetUniformLocation(shader->program, "max_translate");
    glUniform1f(location, max_translate);
    location = glGetUniformLocation(shader->program, "max_rotate");
    glUniform1f(location, max_rotate);
